...
iv <- function(data, y_var, X_vars, Z_vars, intercept = T, hetsked = T) {
  ...
  if (hetsked == T) {
    omega <- lapply(X = 1:n, FUN = function(i) {
      # Define x_i
  ...
  if (hetsked == F) omega <- XX
  # Calculate the standard error
  se <- sqrt(s2 * diag(XX_inv %*% omega %*% XX_inv)) # Vector of _t_ statistics
  t_stats <- (b - 0) / se
  # Calculate the p-values
...
# # Run OLS
iv1 <- iv(card_df, y_vars, X_vars, Z_vars, F) 
...
#ci_95(iv1) %>% knitr::kable(caption = "Return using nearcr as instrument")
...
tsls <- function(data, dep_var, endo_vars, exo_vars, i_vars) {
  ...
  x_star <- ols(data = data, y_data = endo_vars, X_data = X1)$vars$y_hat
  
  namesdf <- names(data)
  data %<>% cbind(x_star) 
  colnames(data) <- c(namesdf, "x_star")
  
  # second stage: Rgress the dependent variable on the exogenous x and x_hat
  X_hat <- unlist(list(c(exo_vars), "x_star"))
  
  s2 <- ols(data, y_data = dep_var, X_data = X_hat)
